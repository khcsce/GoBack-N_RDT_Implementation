    ...
    for (int i = 1; ; i++) {
            ...
            while(1) {
                ...
                if (n > 0) {
                    ...
                    if (ackpkt.seqnum == cliSeqNum && ackpkt.ack && ackpkt.acknum == (synackpkt.seqnum + 1) % MAX_SEQN) {
						...
						//The sequence number of the packet to be received next time
        ...
        while(1) {
            ...
            if (n > 0) {
				if (recvpkt.fin) {
					printRecv(&recvpkt);
                    cliSeqNum = (cliSeqNum + 1) % MAX_SEQN; // sending a fin packet consumes one byte, 
                    /// the sequence number of the next fin packet sent = to the ack returned by the server plus one
				...
				//packet is duplicate or is not the packet expected , discard the packet
				...
				fflush(fp);// flush file cache
				//The sequence number of the packet expected to be received next time
